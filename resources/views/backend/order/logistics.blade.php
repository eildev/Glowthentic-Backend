                                ...
                                @if ($delivering_orders->count() > 0)
                                    @foreach ($delivering_orders as $delivering_orders)
                                        @php
                                            if ($delivering_orders->order->user_id != null) {
                                                $customers = App\Models\UserDetails::where(
                                                    'user_id',
                                                    $delivering_orders->order->user_id,
                                                )->first();
                                            } else {
                                                $customers = App\Models\UserDetails::where(
                                                    'session_id',
                                                    $delivering_orders->order->session_id,
                                                )->first();
                                            }
                                        @endphp
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $delivering_orders->order->payment_status ?? 0 }}</td>
                                            ...
                                            <td>
                                                ...
                                                <a href="{{ route('admin.shipping.order.change.transit', $delivering_orders->id) }}"
                                                    class="btn btn-sm btn-info">In Transit</a>
                                                <a href="{{ route('order.details', $delivering_orders->order->id) }}"
