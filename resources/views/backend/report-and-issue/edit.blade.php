                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="title" id="title" class="form-control"
                                        value="{{ old('title', $report->title) }}" placeholder="Enter issue title">
                                    @error('title')
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="source" id="source" class="form-select">
                                        <option value="" {{ old('source', $report->source) == '' ? 'selected' : '' }}>
                                            Select Source (Default: Website)</option>
                                        <option value="website"
                                            {{ old('source', $report->source) == 'website' ? 'selected' : '' }}>Website
                                        </option>
                                        <option value="mail"
                                            {{ old('source', $report->source) == 'mail' ? 'selected' : '' }}>Mail</option>
                                        <option value="whatsapp"
                                            {{ old('source', $report->source) == 'whatsapp' ? 'selected' : '' }}>WhatsApp
                                        </option>
                                        <option value="facebook"
                                            {{ old('source', $report->source) == 'facebook' ? 'selected' : '' }}>Facebook
                                        </option>
                                        <option value="others"
                                            {{ old('source', $report->source) == 'others' ? 'selected' : '' }}>Others
                                        </option>
                                        <option value="admin"
                                            {{ old('source', $report->source) == 'admin' ? 'selected' : '' }}>Admin
                                        </option>
                                    </select>
                                    @error('source')
                                        <span class="text-danger small">{{ $message }}</span>
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <textarea name="description" id="description" class="form-control" rows="4" placeholder="Enter description">{{ old('description', $report->description) }}</textarea>
                                    @error('description')
                                        <span class="text-danger small">{{ $message }}</span>
                                    @enderror
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <div class="my-1">
                                        <i><b>Note:</b> Please provide a 200x200 image</i>
                                    </div>
                                    @error('attachment')
                                        <span class="text-danger small">{{ $message }}</span>
                                    @enderror
                                    ...
                                    <div class="mt-3">
                                        <img id="showImage" class="img-fluid" height="150" width="200"
                                            src="{{ $report->attachment ? asset($report->attachment) : asset('Uploads/productempty.jpg') }}"
                                            alt="Preview Image">
                                    </div>
                                </div>
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="source_reference" id="source_reference" class="form-control"
                                        value="{{ old('source_reference', $report->source_reference) }}"
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="status" id="status" class="form-select">
                                        <option value=""
                                            {{ old('status', $report->status) == '' ? 'selected' : '' }}>Select Status
                                            (Default: Pending)</option>
                                        <option value="pending"
                                            {{ old('status', $report->status) == 'pending' ? 'selected' : '' }}>Pending
                                        </option>
                                        <option value="in-progress"
                                            {{ old('status', $report->status) == 'in-progress' ? 'selected' : '' }}>In
                                            Progress</option>
                                        <option value="solve"
                                            {{ old('status', $report->status) == 'solve' ? 'selected' : '' }}>Solve
                                        </option>
                                        <option value="issue"
                                            {{ old('status', $report->status) == 'issue' ? 'selected' : '' }}>Issue
                                        </option>
                                        <option value="other"
                                            {{ old('status', $report->status) == 'other' ? 'selected' : '' }}>Other
                                        </option>
                                    </select>
                                    @error('status')
                                        <span class="text-danger small">{{ $message }}</span>
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <textarea name="note" id="note" class="form-control" rows="4" placeholder="Enter note">{{ old('note', $report->note) }}</textarea>
                                    @error('note')
                                        <span class="text-danger small">{{ $message }}</span>
                                    @enderror
                                ...
                                <div class="col-md-12">
                                    <button type="submit" id="submitBtn" class="btn btn-primary">Update Report</button>
                                </div>
                            </div>
                        </form>
                    </div>
